...
//	하지만 이런 방식으로 설계를 하면, 어디에서 원본을 수정했는지 알 수 없다

//	얕은 복사를 사용하면 직접 값을 하나씩 복사해서 오기 때문에
//	다른 곳에서 필드를 수정해도 원본은 바뀌지 않는다.
//	따라서 원본 필드에 선언된 객체가 변경된다면, 얕은 복사된 객체들도 해당 정보는 변경된다.

//	필드 안에 또 다른 객체가 있다면 (객체 그래프) 깊은 복사를 해야 한다.
//	실제 주소를 그대로 받는 것이 아니라 객체 그래프 만큼 new를 사용하여 얕은 복사를 진행해야 한다
//	이렇게 객체 그래프일 경우에는 얕은 복사를 사용하지 않고 깊은 복사를 통해 immutable 하게 만들어야 한다.

	User1 user = new User1("first", "last");
}
